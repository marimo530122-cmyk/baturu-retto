...
| 🛒 三口割りのアフィリエイト枠 | `mikuchiwari-config.js` | ファイル冒頭のコメント参照 |

`mikuchiwari.html`（🍻三口割り：3段階の割り勘計算）はゲーム本体とは独立した1ページだけのツールです。`index.html`や他のJSを読み込まないので、編集しても上記3章の`?v=`を上げる必要はありません。アフィリエイト枠には必ず【PR】表記とAmazonアソシエイトの定型文が付きます（ステマ規制・規約対応のため外さないこと）。
