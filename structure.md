...
| `ADULT_DATA` | 🔞大人向け | 9言語（ja/en/zh/ko/es/pt/vi/de/tl）。fr・th・idが未対応 |
| `FAMILY_DATA` | 👨‍👩‍👧ファミリー | 同上（9言語、fr・th・idが未対応） |
| `COUPLE_DATA` | 💑1対1モード | 同上（9言語、fr・th・idが未対応） |
| `ROMANCE_DATA` | 💌恋愛パック | 同上（9言語、fr・th・idが未対応） |
| `PARTY_DATA` | 🎉法人・パーティー | 同上（9言語、fr・th・idが未対応） |
| `SOLO_DATA` | 🍶ひとり飲み | 同上（9言語、fr・th・idが未対応） |
...
- タゴサクAIは、Google AI StudioでのAPIキー取得でユーザーがつまずいたまま中断中。Vercelへのデプロイ・APIキー設定がまだ完了していない
...
- **⚠️ 有料6パック（ADULT/FAMILY/COUPLE/ROMANCE/PARTY/SOLO）に、フランス語・タイ語・インドネシア語のお題がまだ無い**。この3言語でこれらのパックを選ぶと、購入済みでも中身が標準パックにフォールバックしてしまう（表示テンプレート自体は対応済みなので不具合ではなくコンテンツの抜け）
- **⚠️ 💘ねるとんZoomパック（`NERUTON_DATA`）が日本語・英語の2言語のみ**。他の有料パックが2026-08-10に9言語まで拡張されたのに、これだけ取り残されている
