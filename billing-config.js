/* =========================================================
   💎 プレミアム課金の設定（Stripe決済リンク）
   ---------------------------------------------------------
   ここに、あなた自身のStripe決済リンクを貼り付けると
   プレミアム機能の販売が始まります。
   作り方は 要件定義書.md の「プレミアム課金の設定方法」を参照してください。

   ⚠️ 忘れずに：Stripe側の決済リンク設定で「支払い後」を
   「別のウェブサイトに移動させる」にし、移動先URLを
     https://marimo530122-cmyk.github.io/baturu-retto/?paid=1&session_id={CHECKOUT_SESSION_ID}
   にしてください（{CHECKOUT_SESSION_ID} の部分はそのまま入力すればOK。
   Stripeが決済ごとに自動で本物のセッションIDに置き換えてくれます）。

   まだ設定していない間は、アップグレードボタンを押すと
   「設定が必要です」という案内が表示されるだけで、
   それ以外の機能（無料版・開発確認用の ?premium=1 ）には
   一切影響しません。
   ========================================================= */

const STRIPE_PAYMENT_LINK = "https://buy.stripe.com/00waEY15ceBKcaQg6q5kk00";

/* =========================================================
   🛡️ 決済検証エンドポイントの設定（不正解放の対策）
   ---------------------------------------------------------
   Stripeの決済リンクから戻ってきたURL（?paid=1&session_id=...）は、
   session_idの「形」さえ真似ればブラウザ側だけで偽造できてしまう。
   このエンドポイント（api/verify-payment.js）は、決済から戻った
   直後に「本当にその金額で支払い済みか」をStripe側へ直接問い合わせ、
   偽造だった場合だけ解放を自動的に取り消す（正規の購入者には
   一切見えない、裏側だけの追加チェック）。
   ・api/roast.js と同じVercelプロジェクトに置くだけの関数なので、
     URLは下記の通り最初から設定済み（新しいアカウントやサービスの
     追加は不要）。あなたが行う設定は、Vercel側にStripeの
     「シークレットキー」（STRIPE_SECRET_KEY、Stripeダッシュボードの
     「開発者」→「APIキー」）を環境変数として登録することだけ。
     設定方法は要件定義書.mdの「決済検証エンドポイントの設定方法」参照
   ・⚠️ STRIPE_SECRET_KEYを設定し忘れている間は、エンドポイントが
     「設定エラー」を返す。billing.js側はこれを「検証できなかった
     （＝安全側に倒して解放は取り消さない）」と扱う設計なので、
     設定を忘れていても正規購入者が締め出されることはない。
     ただし本来の目的（不正解放の防止）はその間だけ効かない。
   ========================================================= */

const PAYMENT_VERIFY_ENDPOINT = "https://baturu-retto.vercel.app/api/verify-payment";

/* =========================================================
   🎉 法人・パーティープランの設定（Stripe決済リンク・別売り）
   ---------------------------------------------------------
   結婚式の二次会や会社の飲み会向けの特別パックを、通常の
   プレミアムとは別に単品販売するための設定です。
   通常のプレミアムと同じ手順で、Stripeで「もう1つ」決済リンクを
   作成し、ここに貼り付けてください。

   ⚠️ 忘れずに：Stripe側の決済リンク設定で「支払い後」を
   「別のウェブサイトに移動させる」にし、移動先URLを
     https://marimo530122-cmyk.github.io/baturu-retto/?party_paid=1&party_session_id={CHECKOUT_SESSION_ID}
   にしてください（通常プレミアムの paid / session_id とは
   別のパラメータ名になっているので、間違えないようご注意ください）。

   まだ設定していない間は、法人・パーティープランのボタンを押すと
   「設定が必要です」という案内が表示されるだけで、それ以外の
   機能には一切影響しません。開発確認用に ?partypremium=1 も使えます。
   ========================================================= */

const STRIPE_PARTY_PAYMENT_LINK = "https://buy.stripe.com/YOUR_PAYMENT_LINK_PARTY";

/* =========================================================
   🍶 ひとり飲みモード＋飲み友AI 専用の月額サブスク（¥500/月「ワンコイン」）
   ---------------------------------------------------------
   通常のプレミアム（買い切り）とは別に、🍶ひとり飲みモードと🍶飲み友AIだけを
   月額で提供するためのサブスクです。Stripeで「毎月」の定期支払い設定にした
   決済リンクを作成し、ここに貼り付けてください（作り方は通常のプレミアムと
   同じ手順で、価格設定だけ「Recurring / Monthly」を選ぶ）。

   ⚠️ 忘れずに：Stripe側の決済リンク設定で「支払い後」を
   「別のウェブサイトに移動させる」にし、移動先URLを
     https://marimo530122-cmyk.github.io/baturu-retto/?solo_paid=1&solo_session_id={CHECKOUT_SESSION_ID}
   にしてください（通常プレミアムの paid / session_id とは
   別のパラメータ名になっているので、間違えないようご注意ください）。

   まだ設定していない間は、🍶ひとり飲み・飲み友AIのボタンを押すと
   「設定が必要です」という案内が表示されるだけで、それ以外の
   機能には一切影響しません。開発確認用に ?solopremium=1 も使えます。

   ・QRコードの24時間お試し（お友達紹介特典）は、通常プレミアムと同じく
   このサブスクにも適用されます（全プレミアムに一律適用。billing.jsの
   SoloBillingのallowReferralBonusを参照）。
   ========================================================= */

const STRIPE_SOLO_PAYMENT_LINK = "https://buy.stripe.com/YOUR_PAYMENT_LINK_SOLO";

/* =========================================================
   🪙 飲み友AI：1日の上限に達した後の追加解放（¥100・買い切り）
   ---------------------------------------------------------
   🍶ひとり飲みサブスク内の飲み友AIチャットは1日30回までですが、
   上限に達した後は、この決済リンクで¥100払うとその日だけ+12回
   話せるようになります（何度でも買い足せます）。Stripeで「1回払い」の
   決済リンクを作成し、ここに貼り付けてください（作り方は通常のプレミアムと
   同じ手順）。

   ⚠️ 忘れずに：Stripe側の決済リンク設定で「支払い後」を
   「別のウェブサイトに移動させる」にし、移動先URLを
     https://marimo530122-cmyk.github.io/baturu-retto/?roast_topup=1&roast_topup_session_id={CHECKOUT_SESSION_ID}
   にしてください（他のプランの paid / session_id とは
   別のパラメータ名になっているので、間違えないようご注意ください）。

   まだ設定していない間は、上限に達しても追加購入ボタン自体が
   表示されず、それ以外の機能には一切影響しません。
   ========================================================= */

const STRIPE_ROAST_TOPUP_LINK = "https://buy.stripe.com/YOUR_PAYMENT_LINK_ROAST_TOPUP";
